...
def main():
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            debits_df = df[df["Debit"].notnull()].copy()
            debits_df["Debit"] = debits_df["Debit"].abs()
            credits_df = df[df["Credit"].notnull()].copy()
            ...
            with tab1:
                 ...
                 edited_df = st.data_editor(
                     st.session_state.debits_df[["Transaction Date","Description","Debit","Category"]],
                     column_config={
                         "Debit": st.column_config.NumberColumn("Debit",format="%.2f USD"),
